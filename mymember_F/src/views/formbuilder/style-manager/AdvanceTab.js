...
import EmailAdvanced from "../elements/email/email-advanced";
import BillingAdvanced from "../elements/billing/billing-advanced";
...
const AdvanceTab = (props) => {
  ...
  return (
    <div>
      ...
      {editor?.getSelected()?.attributes?.name === "Billing" ?
        <BillingAdvanced editor={editor} /> : <></>
      }
      ...
      {editor?.getSelected()?.attributes?.name === "MYM TextBox" ?
        <InputAdvance editor={editor} studentList={studentList} membershipList={membershipList}/> : <></>
      }
    </div>
  )
