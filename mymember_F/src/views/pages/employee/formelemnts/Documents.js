...
const Documents = ({
  ...
  const handleselected = (value) => {
    ...
    setselected(value);
    setsubfolder(filterdata[0]);
  };

  const handleselectedSubFolder = (value) => {
    const filterdata = subfolder?.subFolder.filter(
      (item) => item?._id === value
    );
    setselected(value);
    setSelectedSubFolder(filterdata[0]);
  };

  const handleDocIdSelect = (e) => {
    console.log(e)
    roleName?.documentId?.push(e);
    setDocId(e);
  }  
  ...
  //   const filterdata = documentFolderList.filter((item) => item?._id === value);
  //   roleName?.documentId?.push(filterdata[0]?._id);
    ...
    >
      ...
      <div className="d-flex justify-content-end">
        <div className="d-flex justify-content-between">
          ...
          <Button className="m-1" variant="contained" onClick={handleCloseOpen}>
            Add
          </Button>
        </div>
      </div>
...
export default connect(mapstateprops, { GET_DOCUMENT_FOLDER_LIST, CREATE_ROLE_LIST })(Documents);
