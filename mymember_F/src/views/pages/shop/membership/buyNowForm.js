...
class PaymentSummary extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      loading: false,
      valorPayload: {
        stripePaymentMethod: "newCard", // or existingCard
        card_id: "",
        card_number: "",
        amount:
          props.memberShipDetail.down_payment > 0
            ? props.memberShipDetail.down_payment
            : props.memberShipDetail.total_price,
        card_holder_name: "",
        cvv: "",
        expiry_date: "",
        pan: "",
  ...
  BuyNow = async (e) => {
    ...
    if (
      payload.payment_type === "monthly" ||
      payload.payment_type === "weekly"
    ) {
    ...
    let res;
    if (payload?.ptype === "cash") {
      if (this.props.studentList) {
        updatedPayload.membership_details.student_name = `${this.props.studentList?.firstName} ${this.props.studentList?.lastName}`;
      }
      if (this.state.cheque_no) {
        res = await this.props.BUY_MEMBERSHIP_WITH_STRIPE(
          this.props.memberShipDetail,
          updatedPayload,
      ...
      if (
        ...
        updatedPayload.membership_details["stripePayload"] =
          updatedPayload.membership_details["valorPayload"];
        delete updatedPayload.membership_details["valorPayload"];
        updatedPayload.membership_details.stripePayload.card_expiry_month =
          updatedPayload.membership_details.stripePayload.expiry_date.slice(
            0,
            2
          );
        updatedPayload.membership_details.stripePayload.card_expiry_year = `20${updatedPayload.membership_details.stripePayload.expiry_date.slice(
          5
        )}`;
      ...
      } else if (stripePaymentMethod === "existingCard") {
        updatedPayload.membership_details["stripePayload"] =
          updatedPayload.membership_details["valorPayload"];
        delete updatedPayload.membership_details["valorPayload"];
        updatedPayload.membership_details.ptype = "credit card";
                    ...
                    <BuyMemberShipStepperForm
                      ...
                      StripePayment={
                        <StripePayment
                          ...
                          addressChanges={this.addressChanges}
                          HandleChange={this.changeHandler}
                          setStripePayload={this.setStripePayload}
                          loading={this.state.loading}
                          studentId={this.state.studentId}
                        />
                      }
