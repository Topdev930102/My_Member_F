...
const useStyles = makeStyles(() => ({
  ...
  typeSelect: {
    padding: "0.4rem 1.3rem 0.4rem 0.7rem !important",
  },
}));
...
const FormToAddIncalendar = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    let start = moment(payload.start_time.$d).format("YYYY-MM-DD");
    let end = moment(payload.end_time.$d).format("YYYY-MM-DD");
    ...
    tempPayload.start = moment(tempPayload?.start_time.$d).format("MM-DD-YYYY");
    tempPayload.end = moment(tempPayload?.end_time.$d).format("MM-DD-YYYY");
            ...
            <FormGroup className="m-0">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DateTimePicker
                  id="startDate"
                  renderInput={(params) => (
                    <StyledTextField width="100%" {...params} />
                  )}
