...
const AppoinmentAndEvent = (props) => {
  ...
  const {
    ...
    APPOINTMENT_CATEGORYLIST,
    showEventsAppList,
    setShowEventsAppList,
    type,
  } = props;
  const [open, setOpen] = useState(false);
  const [showEvents, setShowEvents] = useState(showEventsAppList);
  ...
  const formatEventList = async () => {
    ...
    setDataAfterFormate(formated);
  };
  useEffect(() => {
    if (eventsAndAppointment?.data !== undefined) {
      formatEventList();
    }
    setShowEvents(showEventsAppList);
  }, [eventsAndAppointment, showEventsAppList]);
  ...
  return (
    <div>
      ...
      <Drawer
        ...
        open={showEvents}
        onClose={() => setShowEventsAppList(false)}
      >
