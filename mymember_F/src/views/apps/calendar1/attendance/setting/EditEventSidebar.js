...
const AddEvent = (props) => {
  ...
  const updateEvent = async (id) => {
    ...
    await props.handleSidebar(false);
  };
  ...
  const UpdateWholeSeries = async () => {
    let payload = {
      ...
      start_time: moment(wholeStartDate).format(),
      end_time: moment(wholeEndDate).format(),
    ...
    if (
      state?.end_time !== wholeEndDate ||
      state?.start_time !== wholeStartDate
    ) {
  ...
  return (
    <>
      ...
      <div className="d-flex justify-content-between">
        <h3 className="text-bold-500 mt-1 ml-1">
