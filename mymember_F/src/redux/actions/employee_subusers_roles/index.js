...
export const CREATE_SUB_USER = (Info, role_id) => {
  return async (dispatch) => {
    try {
      ...
      if (response?.data?.success) {
        toast.success("Sub-User create successfully!", toastCSS());
...
export const UPDATE_EVENT = (Info, id, role_id) => {
  let url = `${baseUrl}/api/sub-users/permissions/update/${id}`;
  return async (dispatch) => {
    try {
      let response = await axios.put(url, Info, getHeaders());
      if (response.data && response.status === 200) {
        toast.success("Sub-user updated successfully!", toastCSS());
        dispatch(GET_SUB_USERS_LIST(role_id));
        return true;
      }
    } catch (error) {
