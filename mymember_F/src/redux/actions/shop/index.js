  ...
  return async (dispatch) => {
    try {
      ...
      if (res?.data?.success) {
        dispatch(GET_MEMBERSHIP_FOLDER_LIST());
        toast.success(
          `Membership Updated successfully!`.replace(/\\/g, ""),
          toastCSS()
        );
      } else {
    ...
    try {
      ...
      if (res.data.success) {
        if (memberShipInfo?.membershipDoc?.length > 0) {
          ...
          let Buy_MSId = res?.data?.data;
          let MS_id = memberShipInfo?._id;
          let urlMerge = `${baseUrl}/api/membership/mergeDoc/${getUserId()}/${StudentId}/${MS_id}/${Buy_MSId}`;
          let result = await axios.post(urlMerge, payload, {
            headers: getHeaders(),
          });
...
export const CREATE_PRODECT_FOLDER = (folderName) => {
  ...
  return async (dispatch) => {
    try {
      let response = await axios.post(url, folderName, {
        headers: getHeaders(),
      });
  ...
  return async (dispatch) => {
    try {
      let response = await axios.get(
        `${baseUrl}/api/member/active_student/${localStorage.getItem(
          "user_id"
        )}`,
        {
          headers: getHeaders(),
