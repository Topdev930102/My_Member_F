...
export const GET_CLASS_SCHEDULE = (class_sh_id) => {
  return async (dispatch) => {
    await axios.post(
      `${baseUrl}/api/class_schedule_by_id/${getUserId()}/${class_sh_id}`,
      {
        headers: getHeaders(),
      }
    );
  ...
  return async (dispatch) => {
    try {
      ...
      if (response.data && response.status === 200) {
        dispatch(
          SUCCESS_SCHEDULE_STATUS("Class schedule is created successfully!")
        );
      } else {
...
export const Get_CAMP_LIST = (data) => {
  return async (dispatch) => {
    try {
      let response = await axios.get(
        `${baseUrl}/api/camp/list_camp/${getUserId()}`,
        {
          headers: getHeaders(),
        }
      );
...
export const Get_SUBUSER_LIST = (data) => {
  return async (dispatch) => {
    try {
      let response = await axios.get(
        `${baseUrl}/api/users/user_list/${getUserId()}`,
        {
          headers: getHeaders(),
        }
      );
  ...
  return async (dispatch) => {
    try {
      let response = await axios.post(
        `${baseUrl}/api/users/add_user/${getUserId()}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
  ...
  return async (dispatch) => {
    try {
      let response = await axios.put(
        `${baseUrl}api/update_stripe/${getUserId()}/${stripeId}`,
        formData,
        {
          headers: getHeaders(),
        }
...
export const trashSubUser = (id) => {
  return (dispatch) => {
    axios
      .delete(`${baseUrl}/api/users/delete_user/${getUserId()}/${id}`, {
        headers: getHeaders(),
      })
      .then((res) => {
        dispatch(Get_SUBUSER_LIST());
...
export const trashCamp = (id) => {
  return (dispatch) => {
    axios
      .delete(`${baseUrl}/api/camp/delete_camp/${getUserId()}/${id}`, {
        headers: getHeaders(),
      })
      .then((res) => {
